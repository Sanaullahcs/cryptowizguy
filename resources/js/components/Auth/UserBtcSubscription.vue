<template>
  <layout>
    <div class="container pb-5 user-padd">
      <div class="col-12 col-sm-12 mw-wwh m-auto col-md-12 col-lg-12 header-sect pb-4">
        <h2>SUBSCRIPTION DETAILS PAGE</h2>
      </div>
      <div v-if="isKickstarterexpiredSub" class="row">
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              <div class="row">
                <div class="col-md-12">
                  <h5 class="card-title">Current Subscription Plan</h5>
                </div>
              </div>
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              ...
              <h3>
                {{
                  btcSubscription ? "Bitcoin" : "No Subscription or  verify your Status"
                }}
              </h3>
              <div class="row pt-3 pb-2">
                <div class="col-md-6">
                  <h4>
                    {{ btcSubscription ? btcSubscription.btc_username : "NA..." }}
                  </h4>
                </div>
                <div class="col-md-6 text-right">
                  <h4>
                    {{ btcSubscription ? btcSubscription.btc_price : "NA..." }}
                    BTC
                  </h4>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <p>
                    {{
                      btcSubscription
                        ? formatDate(btcSubscription.verify_btc_at)
                        : "NA..."
                    }}
                  </p>
                </div>
                <div class="col-md-6 text-right">
                  ...
                  <p>
                    {{
                      btcSubscription
                        ? formatDate(calculateEndDate(btcSubscription.verify_btc_at))
                        : "NA..."
                    }}
                  </p>
                </div>
...
export default {
  ...
  methods: {
    ...
    //     const response = await axios.get("/api/user-subscription-plan", {
    //       headers: {
    ...
    //     this.usersubscription = response.data.subscription.subscriptionsPlan;
    //     this.subscriptiondiscount = response.data.subscription.subscriptiondiscount;
    ...
    //       this.$router.push("/subscription-plan"); // Replace '/dashboard' with your dashboard route
    //       localStorage.removeItem("plan_id");
    //       localStorage.removeItem("subscribed"); // Remove plan_id upon successful cancellation
    //       this.cancellationMessage = "Your subscription has been successfully canceled.";
    ...
    //     });
    // },
    async getBtcSubscription() {
      const userId = this.useridSub; // Ensure this is correctly set and not undefined

      try {
        const response = await axios.post("/api/get-btcSubscription", {
          user_id: userId,
        });

        this.btcSubscription = response.data.btcSubscription;
      } catch (error) {
        console.error(error.response.data);
        this.errorMessage = "Failed to fetch BTC subscription. Please try again.";
      }
    },
    formatDate(timestamp) {
      try {
    ...
    calculateEndDate(startDate) {
      try {
        ...
        const end = new Date(start.setFullYear(start.getFullYear() + 1)); // Add one year to the start date
